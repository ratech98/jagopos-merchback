...
exports.verifyOTP = async (req) => {
  ...
  try {
    ...
    if (findOTP) {
      ...
      const getDevice = await deviceSchema.find({ mac_address, type: "KDS" });
...
exports.clerklogin = async (req) => {
  ...
  try {
    ...
    const findMac = await deviceschema.find({ mac_address, type: "KDS" });
    ...
    const token = await jwt.sign(
      {
        user: "clerk",
        id: id,
        mac_address: findMac[0].mac_address,
        type: "KDS",
      },
    ...
    return {
      ...
      responseCode: 500,
      resultCode: Json.merchant.error.clerk_login.resultCode,
      message: Json.merchant.error.clerk_login.message,
      db_error: error.message,
    };
  }
};
...
exports.resendOtp = async (req) => {
  ...
  try {
    ...
    return {
      ...
      success: true,
      resultCode: Json.merchant.success.merchant_login.resultCode,
      message: Json.merchant.success.merchant_login.message1,
      token,
    };
  } catch (error) {
    return {
